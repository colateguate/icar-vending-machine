...
- **Feature/epic intake** → `product-manager` skill (interview to close the definition before any ticket exists; it produced the configurable-coins epic). **Features** → `implement-feature` skill (TDD, red first, active verification before claiming done). **Bugs/refactors/trivial** → `fix-bug` skill. **Before any push** → `review-before-push` skill (dispatches the 4 reviewer agents in parallel, aggregates a PASS/KO verdict).
