...
- **Mutation score is 95%, not 100%, and it stays that way on purpose.** Around fourteen mutants of the dynamic-programming table survive: shifting the base score, pre-emptying a row that PHP would create anyway, starting a loop counter one below zero. None is observable through the port, because the table's absolute values are an internal ordering device and only the *selection* leaves the method — verified by running the mutated variants over ~14,000 randomised reserves (every total exact, the reserve never over-drawn) and ~4,000 minimality comparisons against an exhaustive oracle. They could be silenced in `infection.json5` the way one equivalent `CastInt` pair already is, and they are not: that suppression is scoped to a single mutator on a single method with a two-mutant proof, whereas muting six mutators across the algorithm's core would also hide the next *real* defect introduced there. A safety net on the most intricate code in the project is worth more than a round number.
  ...
  **This consequence was superseded later in the same ticket, on 2026-08-19.** The paragraph above is left standing rather than rewritten, because the reason it lost is the interesting part.

  The measurement was extended: each of the fourteen surviving mutants was implemented in isolation and run over ~2,300 payable reserves against an exhaustive oracle, and none of them changed a selection the port can observe. That evidence changed the decision. Six mutators are now suppressed in `infection.json5`, and the score is 100%, not 95%.

  What the argument above got wrong was not the principle but the scope it assumed. It rejected suppression on the grounds that muting six mutators across the algorithm's core would hide the next real defect introduced there — and the answer turned out to be to make the suppression narrow enough that it cannot: named mutators on two named methods, never a file or a folder, with the proof written beside them and an expiry condition attached. *Add a non-canonical denomination and those entries must be deleted, because monotonicity is a property of the coin set rather than of the algorithm, and the mutants become killable the moment it breaks.*

  The full reasoning lives in the comment block of `infection.json5` and in [`../testing-strategy.md`](../testing-strategy.md).
