...
**Every response is validated.** `ApiTestCase` sends its request, then checks the response against the document: the status must be declared for that operation, the content type must be one the operation offers, and the body must satisfy the schema. Forty-six test methods across nine classes get this for free, without a single new HTTP call — a hundred and twelve responses checked, and the acceptance suite's assertion count says so, because the check is an assertion rather than a bare `fail()`. A gate that registers nothing when it passes leaves no trace of having run, and "the suite is green" stops being evidence that anything was compared.
...
- **No gate compares, operation by operation, what the code can raise against what the document declares.** Added 2026-08-20, when the document was found declaring the 400 on one of the three operations that answer with it. All three gates are blind to this by construction, and the reason is the part worth reading: the first checks only the responses the suite produces, and none produced that 400; the second crosses the catalogue with the document globally, and `malformed_json` was documented — in another operation; the third checks examples, and the example was correct where it sat. A hole one operation wide fits through all three. Closing it in general means knowing which exceptions each controller can raise, which is not derivable without running it. Closing it here meant asking which request DTOs are built through `JsonBody::of()` and declaring the 400 on exactly those three.
